...
## Load data
```{r}
file <- paste0(path, "data/mcr_pivot_runs.csv")
...
ggsave(plot = ga.pca.combi, filename=paste0(path, 'output/biplots/biplot_combined.pdf'), height = 210*2, width = 297, units = "mm")
ggsave(plot = ga.pca.combi, filename=paste0(path, 'output/biplots/biplot_combined.png'), height = 210*2, width = 297, units = "mm")
...
p.combi
ggsave(filename=paste0(path, 'output/mw_biplot_combined.pdf'), plot=p.combi, width=210, height=297, units='mm')
ggsave(filename=paste0(path, 'output/mw_biplot_combined.png'), plot=p.combi, width=210, height=297, units='mm')
```
...
ggsave(filename=paste0(path, 'output/biplots/human_samples.png'), width=297, height=120, units='mm')  
```
...
## Tree plot 
```{r tree_plot}
gt <- ggtree(tree) + coord_cartesian(clip='off') + theme_tree2() #, layout = 'circular')
...
n_row = 5
year.legend <- make_legend(data=tree.years, x='pos', y='tree.tip.label', fill='collection_year', title='Year', colors=year.colors, nrow=n_row, legend.direction='horizontal', byrow=F)
...
pdf_file <- paste0('output/contig_plots/mcr_tree_window', window, '_scale.pdf')
ggsave(pdf_file, plot=gp8, width=35, height=21, units = 'cm', limitsize = F)
# knitr::plot_crop(pdf_file)
png_file <- paste0('output/contig_plots/mcr_tree_window', window, '.png')
